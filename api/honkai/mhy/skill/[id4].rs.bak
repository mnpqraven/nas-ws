...
use nas_ws::{handler::FromAxumResponse, routes::honkai::mhy_api::internal::skill_by_char_id};
use serde_json::json;
...
pub async fn handler(req: Request) -> Result<Response<Body>, Error> {
    ...
    match id_key {
        ...
        Some(id) => {
            let id: u32 = id.parse()?;
            Ok(skill_by_char_id(Path(id)).await?).as_axum()
        }
    }
}
